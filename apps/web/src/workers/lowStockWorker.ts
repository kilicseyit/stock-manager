 ...
 * Son 24 saat içinde aynı ürün için bildirim gönderilmişse tekrar göndermez.
 */
export function createLowStockWorker() {
  ...
  const worker = new Worker(
    ...
    async (job) => {
      ...
      console.log(`[Worker] ${created} bildirim oluşturuldu. Düşük stok: ${lowStockItems.length} kalem.`);
    },
    { connection }
  );
